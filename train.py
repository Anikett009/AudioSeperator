...
def train(random_seed = 0):
    ...
    train_path = 'MIR-1K/train.txt'
    valid_path = 'MIR-1K/valid.txt'
    ...
    for i in range(iterations):
        ...
        x_mixed = tf.expand_dims(x_mixed, axis=-1) if x_mixed.ndim == 2 else x_mixed
        y1 = tf.expand_dims(y1, axis=-1) if y1.ndim == 2 else y1
        y2 = tf.expand_dims(y2, axis=-1) if y2.ndim == 2 else y2
